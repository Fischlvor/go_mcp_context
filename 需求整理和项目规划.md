...
CREATE TABLE libraries (
    ...
    embedding_model VARCHAR(100) DEFAULT 'text-embedding-3-small',  -- 当前使用的 Embedding 模型
...
CREATE TABLE documents (
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'archived', 'deleted', 'failed')),
    deleted_at TIMESTAMP NULL,
...
CREATE TABLE document_chunks (
    ...
    library_id INTEGER NOT NULL REFERENCES libraries(id) ON DELETE CASCADE,
    chunk_index INTEGER NOT NULL,          -- 块在文档中的位置
    chunk_text TEXT NOT NULL,
    tokens INTEGER,                         -- token 数量
    embedding vector(1536),                 -- OpenAI text-embedding-3-small
    chunk_type VARCHAR(10) DEFAULT 'mixed' CHECK (chunk_type IN ('code', 'info', 'mixed')),  -- 块类型
    access_count INTEGER DEFAULT 0,         -- 访问次数（用于热度计算）
    metadata JSONB,                         -- 块的元数据（标题、代码类型等）
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'archived', 'deleted')),
    deleted_at TIMESTAMP NULL,
...
**字段说明：**
- `tokens` - 用于成本计算和性能监控
- `chunk_type` - 块类型（code/info/mixed），用于 mode 参数过滤
- `access_count` - 访问次数，用于热度计算（重排序权重 20%）
- `metadata` - 存储块的来源信息（如代码语言、标题等）
- `status` - 状态（active、archived、deleted）
